...
!function(a,b){"use strict";function c(c){c=c||{};for(var e in l)l.hasOwnProperty(e)&&(n[e]=c[e]?a.merge(c[e],n[e]):n[e]);n=a.extend(n,c),n.minWidth.sort(h),n.maxWidth.sort(g),n.minHeight.sort(h),n.maxHeight.sort(g);for(var f in l)if(l.hasOwnProperty(f)){k[f]={};for(var i in n[f])if(n[f].hasOwnProperty(i)){var j=b.matchMedia("("+l[f]+": "+(1/0===n[f][i]?1e5:n[f][i])+n.unit+")");j.addListener(d),k[f][n[f][i]]=j}}d()}function d(){e(),a(b).trigger("snap",[j])}function e(){j={unit:n.unit};for(var a in l)if(l.hasOwnProperty(a))for(var b in k[a])if(k[a].hasOwnProperty(b)&&k[a][b].matches){var c="Infinity"===b?1/0:parseInt(b,10);a.indexOf("max")>-1?(!j[a]||c<j[a])&&(j[a]=c):(!j[a]||c>j[a])&&(j[a]=c)}}function f(a){var b=m[a.media],c=a.matches?"enter":"leave";if(b&&(b.active||!b.active&&a.matches)){for(var d in b[c])b[c].hasOwnProperty(d)&&b[c][d].apply(b.mq);b.active=!0}}function g(a,b){return b-a}function h(a,b){return a-b}var i=void 0!==b.matchMedia,j=null,k={},l={minWidth:"min-width",maxWidth:"max-width",minHeight:"min-height",maxHeight:"max-height"},m=[],n={minWidth:[0],maxWidth:[1/0],minHeight:[0],maxHeight:[1/0],unit:"px"},o={bind:function(a,c){m[a]||(m[a]={mq:b.matchMedia(a),active:!1,enter:[],leave:[]},m[a].mq.addListener(f));for(var d in c)c.hasOwnProperty(d)&&m[a].hasOwnProperty(d)&&m[a][d].push(c[d]);return f(m[a].mq),this},defaults:function(b){n=a.extend(n,b||{})},state:function(){return j},unbind:function(a){return m[a]&&(m[a].mq.removeListener(f),m=m.splice(m.indexOf(m[a]),1)),this}};a.rubberband=function(a){if(i){if(o[a])return o[a].apply(this,Array.prototype.slice.call(arguments,1));if("object"==typeof a||!a)return c.apply(this,arguments)}return this}}(jQuery,window);