...
(function($) {
		...
		_doRespond: function() {
			...
			for (var point in settings.breakpoints) {
				...
				if (settings.breakpoints[point + 1]) {
					if (w <= settings.breakpoints[point] && w >= settings.breakpoints[point + 1]) {
				...
				if (w <= settings.breakpoints[point] && (w > settings.breakpoints[point + 1] || typeof settings.breakpoints[point + 1] === "undefined")) {
	...
	$.fn.rubberband = function(method) {
		if (methods[method]) {
			...
			$.error("RESPONDER: '" +  method + "' does not exist.");
		} 
	};
})(jQuery);